...
function ArticleByID() {
  ...
  const addComment = async (commentObj) => {
    try {
      ...
      const res = await axios.post("http://localhost:4000/user-api/articles/comment", payload, { withCredentials: true });
      if (res.status === 200) {
        toast.success(res.data.message);
        setArticle(res.data.payload);
        reset({ comment: "" });
  ...
  const toggleCommentLike = async (commentId) => {
    try {
      ...
      const res = await axios.post("http://localhost:4000/user-api/articles/comment/like", payload, {
        withCredentials: true,
      });
      if (res.status === 200) {
        toast.success(res.data.message);
      ...
      toast.error(err.response?.data?.message || err.response?.data?.error || "Failed to update like");
    }
  };
      ...
      <div className="mt-8">
        ...
        {(article.comments || []).map((comment, index) => {
          ...
          const userLiked = currentUserId
            ? likedBy.some((id) => String(id) === String(currentUserId))
            : false;
              ...
              <div className="mt-4 flex items-center justify-between gap-3 text-sm text-slate-600">
                ...
                {user?.role === "USER" && (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full border ${userLiked ? "bg-amber-600 text-white border-amber-600" : "border-slate-300 text-slate-700"}`}
                  >
